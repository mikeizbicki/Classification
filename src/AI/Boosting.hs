...
{-        f0 :: DataPoint -> Bool
        ...
        model0 :: KNN Bool
        model0 = KNN.train (defKNN {k=1}) ls
...
train algcreator param ls us = do
    ...
    where 
        ...
        weightsVec = normalizeVector $ V.fromList     $ [ _D0 alg i | i <- indices alg ]
        labelsVec = V.fromList      $ [ _y0 alg i | i <- indices alg ]
        dataVec = VB.fromList       $ [ d | (l,d) <- ls ]++[d | d<-us]
        
        model = genModelFromDistribution 0 param td
...
--           model = (trainer param) sampledist
          model = genModelFromDistribution seed param td
          f = classify model
